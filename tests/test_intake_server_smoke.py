"""Smoke tests for the intake WSGI server bindings."""
...
def test_intake_server_smoke(tmp_path) -> None:
    host = "127.0.0.1"
    ...
    try:
        ...
        assert "Project NEO Agent Intake" in body

        payload = json.dumps({}).encode("utf-8")
        req = request.Request(
            f"http://{host}:{port}/api/profile/validate",
            data=payload,
            method="POST",
