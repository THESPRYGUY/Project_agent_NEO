...
from neo_build.contracts import (
...
def test_20_pack_presence_and_integrity(tmp_path: Path) -> None:
    ...
    # KPI sync 11/14/17
    k11 = (
        json.loads((repo_path / "11_Workflow-Pack_v2.json").read_text(encoding="utf-8"))
        .get("gates", {})
        .get("kpi_targets")
    )
    k14 = json.loads(
        (repo_path / "14_KPI+Evaluation-Framework_v2.json").read_text(encoding="utf-8")
    ).get("targets")
    k17 = (
        json.loads(
            (repo_path / "17_Lifecycle-Pack_v2.json").read_text(encoding="utf-8")
        )
        .get("gates", {})
        .get("kpi_targets")
    )
    ...
    rules = json.loads(
        (repo_path / "03_Operating-Rules_v2.json").read_text(encoding="utf-8")
    )
    ...
    readme_map = json.loads(
        (repo_path / "01_README+Directory-Map_v2.json").read_text(encoding="utf-8")
    )
