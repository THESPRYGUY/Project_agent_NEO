...
def test_identity_generate_and_persona_config_state(tmp_path: Path):
    ...
    app = create_app(base_dir=tmp_path)
    # identity generate
    payload = json.dumps({"naics_code": "541110", "business_function": "legal", "role_code": "AIA-P", "agent_name": "Neo"}).encode("utf-8")
    ...
    st, _, raw = _call(app, "POST", "/api/persona/state", body=json.dumps({"tone": "crisp"}).encode("utf-8"))
    assert st == "200 OK"
