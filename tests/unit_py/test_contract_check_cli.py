...
def test_contract_check_cli_detects_missing(tmp_path: Path) -> None:
    ...
    (packs_dir / "01_README+Directory-Map_v2.json").write_text(json.dumps({"meta": {}}, indent=2), encoding="utf-8")

    cp = subprocess.run([sys.executable, "scripts/contract_check.py", str(packs_dir)], capture_output=True, text=True)
