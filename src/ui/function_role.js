(function () {
  ...
  function roleMatchesFunction(role, fn) {
    return role.function && role.function.toLowerCase() === fn.toLowerCase();
  }
  ...
  function roleMatchesQuery(role, query) {
    ...
    const haystack = [role.code, role.seniority, ...(role.titles || [])].join(' ').toLowerCase();
    return query
      .toLowerCase()
      .split(/\s+/)
      .filter(Boolean)
  ...
  function updateHiddenValues(role) {
    ...
    if (hiddenRoutingDefaults) {
      hiddenRoutingDefaults.value = JSON.stringify(defaults);
    }
  }
  ...
  function updatePreview(role) {
    ...
    const mergedBootstrap = (window.__FUNCTION_ROLE_STATE__ && window.__FUNCTION_ROLE_STATE__.routing_defaults_json) ? (() => { try { return JSON.parse(window.__FUNCTION_ROLE_STATE__.routing_defaults_json) } catch(_) { return null } })() : null;\n    const defaults = mergedBootstrap || resolveDefaults(functionSelect.value, role.code);
    ...
    if (defaults.workflows.length) {
      renderList('Workflows', defaults.workflows);
    }
    if (defaults.connectors.length) {
      renderList('Connectors', defaults.connectors);
    }
    if (defaults.report_templates.length) {
      renderList('Report Templates', defaults.report_templates);
    }
  ...
  function updateRoleOptions() {
    ...
    filtered.forEach((role) => {
      roleSelect.append(buildRoleOption(role));
    });
