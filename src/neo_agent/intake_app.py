...
"""
        ...
        # Compute effective routing defaults using precedence: baseline < function < role
        try:
            ...
            if role_code_value:
                for r in roles_catalog:
                    if isinstance(r, Mapping) and _str(r.get("code", "")) == role_code_value:
                        candidate = r.get("defaults")
                        if isinstance(candidate, Mapping):
                            role_defaults = candidate
                        break
    ...
    def _dispatch_api(
        ...
        # path normalization is performed above; additional logging removed
